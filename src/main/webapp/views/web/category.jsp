...
<section class="jumbotron text-center">
	<div class="container">
		<h1 class="jumbotron-heading">Filtro</h1>
						...
						<ul class="pagination">
							<c:if test="${index >1}">
								<li class="page-item"><a class="page-link"
									href="category?categoryID=${tagActive}&index=${index-1}">Previous</a></li>
							</c:if>
							<c:forEach begin="1" end="${endPage}" var="i">
								<li class="page-item ${index==i ? "active" : "" }"><a
									class="page-link"
									href="category?categoryID=${tagActive}&index=${i}">${i}</a></li>
							</c:forEach>
							<c:if test="${index < endPage }">
								<li class="page-item"><a class="page-link"
									href="category?categoryID=${tagActive}&index=${index + 1 }">Next</a></li>
							</c:if>
						</ul>
					</nav>
				</div>
		...
		<div class="col-12 col-sm-3">
			<div class="card bg-light mb-3">
				...
				<ul class="list-group category_block">
					<c:forEach items="${listCate}" var="list">
						<li class="list-group-item ${tagActive==list.categoryID ? "active" : ""}"><a
							href="category?cateID=${list.categoryID}">${list.categoryName}</a></li>
					</c:forEach>
				</ul>
			</div>
			<div class="card bg-light mb-3">
